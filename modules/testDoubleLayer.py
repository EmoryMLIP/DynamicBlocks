...
paramsStruct = {'normLayer': nn.BatchNorm2d(num_features=nChan),
                'conv': nn.Conv2d(in_channels=nChan, out_channels=nChan, kernel_size=3, padding=1, stride=1)}

net = DoubleLayer(vFeat, params=paramsStruct)
net.to(device)
...
# ----------------------------------------------------------------------
def compareFunc(x, K1, K2 ,device): # functional DoubleLayer
    z = conv3x3(x, K1)
